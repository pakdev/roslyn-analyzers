...
namespace Analyzers.Utilities.UnitTests
{
    ...
    public sealed class AdditionalFileProviderTests
    {
        ...
        private static FakeAdditionalText[] CreateAdditionalFiles(params (string FileName, string Content)[] fileNameAndContentGroups)
            => fileNameAndContentGroups.Select(x => CreateAdditionalFile(x.FileName, x.Content)).ToArray();

        private static FakeAdditionalText[] CreateAdditionalFiles(params string[] fileNames)
            => fileNames.Select(x => CreateAdditionalFile(x)).ToArray();

        private static FakeAdditionalText CreateAdditionalFile(string fileName, string content = "")
